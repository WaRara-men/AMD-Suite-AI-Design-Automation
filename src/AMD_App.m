% ==========================================
% Algo-Mech Designer (AMD) Suite - App v19.0
% IMMORTAL TAG-BASED ARCHITECTURE
% ==========================================
...
function AMD_App()
    ...
    fig = uifigure('Name', 'AMD Suite v19.0 - THE IMMORTAL ARCHITECT', 'Position', [50 50 1350 750], 'Color', bg_color);
    current_lang = 'JP';

    % --- Header ---
    uilabel(fig, 'Text', '💎 AMD SUITE v19.0: ULTIMATE DESIGN CENTER', 'FontSize', 22, 'FontWeight', 'bold', 'Position', [20 705 600 35], 'FontColor', [0.3 0.8 1.0]);
    btn_git = uibutton(fig, 'push', 'Text', '🌐 SYNC TO GITHUB', 'Position', [1150 710 150 30], 'BackgroundColor', [0.2 0.2 0.2], 'FontColor', 'w');

    % --- 🌟 Tab Group (Managed by TAG) ---
    ...
    % Helper to Build Tabs with Explanations
    ...
    % Results Panel
    p_ana = uipanel(fig, 'Title', 'AI Result', 'Position', [435 120 380 570], 'BackgroundColor', panel_bg, 'ForegroundColor', txt);
    ...
    p_3d = uipanel(fig, 'Title', 'Live 3D View', 'Position', [830 120 480 570], 'BackgroundColor', panel_bg, 'ForegroundColor', txt);
    ...
    % --- 🎯 Master Sync Logic (TAG BASED) ---
    function sync(~, ~)
        try
            mode = tg.SelectedTab.Tag; % 🌟 USE TAG, NOT TITLE!
            c = ctrls.(mode);
    ...
    btn_git.ButtonPushedFcn = @(btn, event) system(sprintf('cd "%s" && git add . && git commit -m "Final v19.0" && git push', project_root));
