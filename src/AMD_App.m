% ==========================================
% Algo-Mech Designer (AMD) Suite - App v18.0
% TRUE INTELLIGENT SWITCH: Reactive Reloading
% ==========================================
...
function AMD_App()
    ...
    fig = uifigure('Name', 'AMD Suite v18.0 - Intelligent Robot Center', 'Position', [50 50 1350 750], 'Color', bg_color);
    current_lang = 'JP';

    % --- Header ---
    uilabel(fig, 'Text', '💎 AMD SUITE: INTELLIGENT DESIGN CENTER', 'FontSize', 22, 'FontWeight', 'bold', 'Position', [20 705 600 35], 'FontColor', [0.3 0.8 1.0]);
    ...
    ctrls = struct();
    function [p, s] = build_tab(tab, title, desc, s_labels, s_hints, s_limits, s_defaults)
    ...
    [~, ctrls.Arm] = build_tab(tab_arm, 'Arm', 'アームのトルクと自重を計算', {'1. Payload [kg]', '2. Length [mm]', '3. Radius [mm]', '4. Budget [JPY]'}, {'→ 荷物の重さ', '→ 腕の長さ', '→ 腕の太さ', '→ 予算上限'}, [0.1 10; 50 1000; 2 50; 1000 50000], [2, 300, 10, 15000]);
    [~, ctrls.Lift] = build_tab(tab_lift, 'Lift', '昇降機の巻上トルクを計算', {'1. Payload [kg]', '2. Pulley Rad [mm]', '3. (N/A)', '4. Budget [JPY]'}, {'→ 吊り上げる重さ', '→ 滑車の半径', '→ ---', '→ 予算上限'}, [0.1 50; 10 100; 0 1; 1000 50000], [5, 30, 0, 15000]);
    [~, ctrls.Mobile] = build_tab(tab_mobile, 'Mobile', '走行車輪の駆動力を計算', {'1. Weight [kg]', '2. Wheel Rad [mm]', '3. Incline [deg]', '4. Budget [JPY]'}, {'→ ロボット総重量', '→ タイヤの半径', '→ 坂の角度', '→ 予算上限'}, [1 100; 20 200; 0 45; 1000 50000], [20, 50, 15, 15000]);
    [~, ctrls.Power] = build_tab(tab_power, 'Power', 'バッテリー容量の選定', {'1. Current [A]', '2. Time [h]', '3. Voltage [V]', '4. Budget [JPY]'}, {'→ 消費電流', '→ 稼働時間', '→ システム電圧', '→ 予算上限'}, [0.1 20; 0.5 24; 5 24; 1000 50000], [2, 3, 11.1, 15000]);
    [~, ctrls.Bolt] = build_tab(tab_bolt, 'Bolt', 'ネジのせん断強度を計算', {'1. Load [kg]', '2. Count', '3. (N/A)', '4. Budget [JPY]'}, {'→ 荷重', '→ ネジの本数', '→ ---', '→ 予算上限'}, [10 1000; 1 16; 0 1; 100 5000], [200, 4, 0, 1000]);
    ...
    lbl_status = uilabel(pnl_ana, 'Text', 'Best: ---', 'FontSize', 16, 'FontWeight', 'bold', 'Position', [20 510 340 40], 'FontColor', [0.2 0.8 1.0]);
    ...
    btn_git.ButtonPushedFcn = @(btn, event) system(sprintf('cd "%s" && git add . && git commit -m "Final v18.0" && git push', project_root));

    % Listeners
    fnames = fieldnames(ctrls);
    for j=1:length(fnames), for s=ctrls.(fnames{j}), addlistener(s, 'ValueChanged', @master_sync); end; end
    addlistener(tg, 'SelectionChanged', @master_sync); % 🌟 THIS IS THE KEY!
    master_sync(); % Initial draw
end
