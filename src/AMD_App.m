% ==========================================
% Algo-Mech Designer (AMD) Suite - App v5.4
% Ultimate Pro Dashboard: Auto-Scaling 3D
% ==========================================
...
function AMD_App()
    ...
    fig = uifigure('Name', 'AMD Suite v5.4 - Real-Time CAE Suite', 'Position', [100 100 1150 600], 'Color', bg_color);
    current_lang = 'JP';

    % --- UI Setup ---
    btn_lang = uibutton(fig, 'push', 'Text', '🌐 Switch to English', 'Position', [1000 560 130 30], 'BackgroundColor', [0.2 0.2 0.2], 'FontColor', 'w');
    pnl_settings = uipanel(fig, 'Title', 'Settings', 'Position', [20 120 300 430], 'BackgroundColor', panel_bg, 'ForegroundColor', txt_color);
    sld_load = uislider(pnl_settings, 'Limits', [5 100], 'Value', 30, 'Position', [20 350 250 3]);
    sld_budget = uislider(pnl_settings, 'Limits', [500 20000], 'Value', 5000, 'Position', [20 250 250 3]);

    pnl_ana = uipanel(fig, 'Title', 'Analytics', 'Position', [330 120 340 430], 'BackgroundColor', panel_bg, 'ForegroundColor', txt_color);
    ...
    pnl_3d = uipanel(fig, 'Title', 'Live 3D Preview', 'Position', [680 120 450 430], 'BackgroundColor', panel_bg, 'ForegroundColor', txt_color);
    ax_3d = uiaxes(pnl_3d, 'Position', [10 10 430 380], 'Color', [0 0 0], 'XColor', 'none', 'YColor', 'none');
    view(ax_3d, 3); axis(ax_3d, 'equal'); grid(ax_3d, 'on'); camlight(ax_3d, 'headlight');

    btn_run = uibutton(fig, 'push', 'Text', '🚀 RUN ANALYSIS & SYNC 3D', 'FontSize', 16, 'FontWeight', 'bold', 'BackgroundColor', [0.5 0.1 0.1], 'FontColor', 'white', 'Position', [100 30 950 70]);

    % --- 💎 Advanced 3D Rendering with Auto-Zoom ---
    function update_3d_view(final_t, mat_name)
        cla(ax_3d);
        stl_file = fullfile(output_dir, 'View_in_3D.stl');
        col = [0.7 0.7 0.8]; if contains(mat_name, 'Steel'), col = [0.4 0.4 0.5]; elseif contains(mat_name, 'Carbon'), col = [0.1 0.1 0.1]; end
        ...
        if exist(stl_file, 'file')
            ...
            catch, draw_fallback(final_t, col); end
        else, draw_fallback(final_t, col); end
        
        % 🌟 AUTO-ZOOM: モデルに合わせてカメラ位置を自動調整
        axis(ax_3d, 'tight'); axis(ax_3d, 'equal');
        camlight(ax_3d, 'headlight'); material(ax_3d, 'shiny');
    end

    function draw_fallback(t, col)
    ...
    function run_all()
        btn_run.Text = '⌛ Syncing with SolidWorks...'; btn_run.Enable = 'off'; drawnow;
        AMD_Main_Brain(sld_load.Value, sld_budget.Value, 1.5, current_lang);
        update_ui(); 
        btn_run.Text = '🚀 RUN ANALYSIS & SYNC 3D'; btn_run.Enable = 'on';
    end
