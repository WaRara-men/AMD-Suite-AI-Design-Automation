% ==========================================
% Algo-Mech Designer (AMD) Suite - App v20.0
% THE ETERNAL ZENITH: Zero-Error Architecture
% ==========================================
...
function AMD_App()
    ...
    fig = uifigure('Name', 'AMD Suite v20.0 - THE ETERNAL ZENITH', 'Position', [50 50 1350 750], 'Color', bg_color);
    current_lang = 'JP';

    % --- Top Bar ---
    uilabel(fig, 'Text', '💎 AMD SUITE v20.0: ULTIMATE DESIGN CENTER', 'FontSize', 22, 'FontWeight', 'bold', 'Position', [20 705 600 35], 'FontColor', [0.3 0.8 1.0]);
    ...
    function s = build_pnl(tab, title, desc, s_labels, s_hints, s_limits, s_defaults)
        ...
        for i = 1:4
            ...
            uilabel(p, 'Text', s_hints{i}, 'Position', [20 y(i)-15, 300, 15], 'FontColor', hint_color, 'FontSize', 10);
            sl = uislider(p, 'Limits', s_limits(i,:), 'Value', s_defaults(i), 'Position', [30 y(i)-30, 300, 3], 'FontColor', 'w');
            s = [s, sl];
        end
    end

    % Tabs Reconstruction (GUARANTEED CONTENT)
    ctrls.Arm = build_pnl(tab_arm, 'Arm', 'アームの旋回トルクと自重を計算', {'Payload [kg]', 'Length [mm]', 'Radius [mm]', 'Budget [JPY]'}, {'→ 荷物の重さ', '→ 腕の長さ', '→ 腕の太さ', '→ 予算上限'}, [0.1 10; 50 1000; 2 50; 1000 50000], [2, 300, 10, 15000]);
    ctrls.Lift = build_pnl(tab_lift, 'Lift', '昇降機の巻上トルクを計算', {'Payload [kg]', 'Pulley Rad [mm]', '---', 'Budget [JPY]'}, {'→ 吊り上げる重さ', '→ 滑車の半径', '→ 予備', '→ 予算上限'}, [0.1 50; 10 100; 0 1; 1000 50000], [5, 30, 0, 15000]);
    ctrls.Mobile = build_pnl(tab_mobile, 'Mobile', '走行車輪の駆動力を計算', {'Weight [kg]', 'Wheel Rad [mm]', 'Incline [deg]', 'Budget [JPY]'}, {'→ ロボット総重量', '→ タイヤの半径', '→ 坂の角度', '→ 予算上限'}, [1 100; 20 200; 0 45; 1000 50000], [20, 50, 15, 15000]);
    ctrls.Power = build_pnl(tab_power, 'Power', 'バッテリー容量の選定', {'Current [A]', 'Time [h]', 'Voltage [V]', 'Budget [JPY]'}, {'→ 消費電流', '→ 稼働時間', '→ システム電圧', '→ 予算上限'}, [0.1 20; 0.5 24; 5 24; 1000 50000], [2, 3, 11.1, 15000]);
    ctrls.Bolt = build_pnl(tab_bolt, 'Bolt', 'ネジのせん断強度を計算', {'Load [kg]', 'Count', '---', 'Budget [JPY]'}, {'→ 荷重', '→ ネジの本数', '→ 予備', '→ 予算上限'}, [10 1000; 1 16; 0 1; 100 5000], [200, 4, 0, 1000]);

    % Analysis & 3D (FIXED ForegroundColor variable)
    p_ana = uipanel(fig, 'Title', 'AI Result', 'Position', [435 120 380 570], 'BackgroundColor', panel_bg, 'ForegroundColor', txt_color);
    ...
    p_3d = uipanel(fig, 'Title', 'Live 3D View', 'Position', [830 120 480 570], 'BackgroundColor', panel_bg, 'ForegroundColor', txt_color);
    ...
    btn_git.ButtonPushedFcn = @(btn, event) system(sprintf('cd "%s" && git add . && git commit -m "Final v20.0" && git push', project_root));
